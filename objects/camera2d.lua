...
camera2d.camera_shake_enabled = true
...
function camera2d:new(target_position)
  ...
  target:set_max_speed(1000)
  target:set_force(1000)
...
function camera2d:_update_shakes(dt)
  ...
  for i=#shakes,1,-1 do
    ...
    if shake.is_finished then
	  ...
	  self.camera_shake_enabled = false
    end
  end
end
...
function camera2d:_get_shake_offset()
  ...
  for i=1,#shakes do
    xoff = xoff + math.random(-1, 1) --shakes[i].xoff
    yoff = yoff + math.random(-1, 1) --shakes[i].yoff
  end
