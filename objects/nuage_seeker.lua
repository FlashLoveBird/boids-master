...
function nu_sk:new(x, y, z)
  ...
  nu_sk.direction = {x = 0, y = 1, z = 0}
  nu_sk.velocity = {x = 0, y = 0, z = 0}
  nu_sk.acceleration = {x = 0, y = 0, z = 0}
  nu_sk.force = {x = 0, y = 0, z = 0}
  nu_sk.target = {x = 0, y = 0, z = 0}
  nu_sk.boundary_bbox = bbox:new(0, 0, 0, 0)
  
  nu_sk.current_angle = math.atan2(nu_sk.direction.y, nu_sk.direction.x)
...
function nu_sk:set_position(x, y, z)
  vector3.set(self.position, x, y, z)
end

function nu_sk:set_direction(dx, dy, dz)
...
function nu_sk:set_target(x, y, z)
  vector3.set(self.target, x, y, z)
end

function nu_sk:add_force(fx, fy, fz)
  vector3.add(self.force, fx, fy, fz)
end

function nu_sk:set_mass(m)
...
function nu_sk:get_rotation_angle()
  return self.rotation_angle
end

function nu_sk:get_roll_speed()
  return self.angle_speed
end

function nu_sk:get_pitch_angle()
  return self.current_zangle
end

function nu_sk:get_position()
  return self.position.x, self.position.y, self.position.z
end

function nu_sk:get_direction()
  return self.direction.x, self.direction.y, self.direction.z
end

function nu_sk:set_bounds(x, y, width, height, depth)
...
--[[----------------------------------------------------------------------]]--
--##########################################################################--
function nu_sk:_update_position(dt)
...
function nu_sk:_update_boundary()
...
function nu_sk:_update_steering(dt)
  local pos = self.position
...
function nu_sk:_update_direction(dt)
  -- calculate direction
  local vel = self.velocity
